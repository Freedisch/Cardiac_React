						...
						<tbody className="bg-gray-700 divide-y divide-gray-600">
							{diagnosisData.length === 0 ? (
								...
								diagnosisData.map((diagnosis, index) => (
									<tr key={diagnosis.id}>
										<td className="px-4 py-3">{index + 1}</td> {/* Row number column */}
										<td className="px-4 py-3">{diagnosis.id}</td> {/* Firestore Document ID */}
										<td className="px-4 py-3">{diagnosis.diagnosis || "N/A"}</td>
										<td className="px-4 py-3">{diagnosis.ECG_Classification || "N/A"}</td>
										<td className="px-4 py-3">{diagnosis.recommendation || "N/A"}</td>
										<td className="px-4 py-3">{diagnosis.followUp || "N/A"}</td>
										<td className="px-4 py-3">{diagnosis.criticalAlert || "N/A"}</td>
									</tr>
								))
							)}
						</tbody>
